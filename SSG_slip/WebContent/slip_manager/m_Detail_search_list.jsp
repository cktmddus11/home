...
PreparedStatement stmt = null;
	...
	System.out.println(sk+"m_Detail_search_list.jsp");
	System.out.println(sv+"m_Detail_search_list.jsp");
    System.out.println(num+"m_Detail_search_list.jsp");
	...
	try{
		...
		if(sk==null & sv==null){
			...
			pstmt = conn.prepareStatement("select id, yoil, lecturenum,lecturename, professor, str_to_date(start, '%H%i%s'), str_to_date(end, '%H%i%s'), start, end from test where "+sk+"=? && lecturenum="+num+" order by id desc");
		...
		for(int k=1; k<=pagesize; k++) {
	        ...
	        /* out.println("건물:"+building+"학과:"+dept+"강의실:"+lecturenum+"강의명"+lecturename+"시작시간:"+start+"종료시간"+end); */	        
	         ...
	         int number = 0;
	           ...
	           if(yoil.equals("일")) number = 1; // ("월", equals(yoil))
	 ...
	 out.println("<a href=m_Detail_search_list.jsp?mypage=" + l+ ">"+ l + "|</a>");
	   }
	}
	else{
	   out.println("1"); 
